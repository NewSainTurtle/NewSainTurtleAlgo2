...
7. `moveMoster`함수를 통해 몬스터를 이동시킨다.
...
8. `movePackmen`함수를 통해 팩맨을 이동시킨다.
...
public class Main_팩맨 {
	...
	static int packmen_i, packmen_j;
	static int[][][] map;
	static int[][][] egg;
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(countMoster());
...
//		System.setIn(new FileInputStream("src/input.txt"));
		...
		packmen_i = Integer.parseInt(st.nextToken()) - 1;
		packmen_j = Integer.parseInt(st.nextToken()) - 1;
			...
			for (int j = 0; j < 4; j++) {
				for (int k = 0; k < 8; k++) {
					if (0 < map[i][j][k]) {
						...
						while (!check(ni, nj) || (packmen_i == ni && packmen_j == nj) || 0 < deadMonsters[ni][nj][1]
	...
	static void movePackmen() {
		int max = -1;
		int[][] seq = new int[3][2];
		for (int i = 0; i < 7; i += 2) {
			int ni = packmen_i + di[i];
			int nj = packmen_j + dj[i];
